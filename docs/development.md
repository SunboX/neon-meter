...
The installer is built for the M5Stack CoreS3 ESP32-S3 target and flashes the
factory firmware generated by the project's GitHub Actions workflow.
...
validates the installer metadata, builds the `m5stack-cores3` firmware, copies
`firmware.factory.bin` into the published artifact, and deploys the page with
GitHub Pages. Web Serial requires the HTTPS GitHub Pages URL; opening the static
